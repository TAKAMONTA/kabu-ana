...
export const RSS_SOURCES: RssSource[] = [
  ...
  },
  {
    id: "argus",
    name: "Argus Media",
    url: "https://www.argusmedia.com/en/rss",
    weight: 0.9,
  },
  {
  ...
  },
  {
    id: "jogmec",
    name: "JOGMEC",
    url: "https://www.jogmec.go.jp/news/release/news.xml",
    weight: 0.7,
  },
  {
    id: "reuters-world",
...
export function parseRssFeed(xml: string, source: RssSource): EnergyNewsItem[] {
  ...
  return itemMatches
    .map((itemXml, index) => {
      ...
      const link =
        tagValue(itemXml, "link") ||
        itemXml.match(/<link[^>]*href="([^"]+)"/i)?.[1] ||
        "";
      const publishedRaw =
        tagValue(itemXml, "pubDate") ||
        tagValue(itemXml, "updated") ||
        tagValue(itemXml, "published");
      const summary =
        tagValue(itemXml, "description") ||
        tagValue(itemXml, "summary") ||
        tagValue(itemXml, "content:encoded");
      ...
      const label: EnergyNewsItem["label"] =
        score >= 7 ? "Critical" : score >= 4 ? "Hot" : "Normal";
      return {
        id: `${source.id}-${Buffer.from(`${link || title}-${index}`)
          .toString("base64url")
          .slice(0, 24)}`,
        title,
