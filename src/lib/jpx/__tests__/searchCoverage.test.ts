 ...
 * 短い名称＋正規化名の完全一致＋独立した4桁トークン）＋別名保持銘柄＋
 * 実害が観測された明示ケースを回す。等間隔サンプルだけでは、このスイートが
 * 守るべき「短名称・被包含名称のカバー」をほとんど見ない（実測: リスクコホート
 * 246件（equity 222件 ∪ non-equity 24件）のうち等間隔サンプルがカバーできるのは
 * 14件＝5.7%に過ぎない）ため、リスクコホートを必ず和集合として加える。
...
// （名称衝突の部分文字列関係＋短い名称＋正規化名の完全一致＋独立した4桁トークン。
// このスイートが守りたいリスクの本体）。
